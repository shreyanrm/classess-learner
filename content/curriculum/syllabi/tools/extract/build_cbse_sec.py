...
from extract import chapter_objectives, blocks
...
CH9M = ["Number System", "Introduction to Polynomials", "Sequences and Progressions",
        "Exploring Algebraic Identities", "Linear Equations in Two Variables",
        "Coordinate Geometry", "Introduction to Euclid's Geometry: Axioms and Postulates",
        "Lines and Angles", "Triangles - Congruence Theorems", "4-gons (Quadrilaterals)",
        "Circles", "Area and Perimeter", "Surface Area and Volume", "Statistics",
        "Introduction to Probability"]
assert len(g) == len(CH9M), (len(g), len(CH9M))
OBJ9M = {name: (page, [tidy(x) for x in items]) for name, (page, items) in zip(CH9M, g)}

U9M = [("Number System", 7, 12, ["Number System"]),
       ("Algebra", 20, 66, ["Introduction to Polynomials", "Sequences and Progressions",
                            "Exploring Algebraic Identities", "Linear Equations in Two Variables"]),
       ("Coordinate Geometry", 4, 6, ["Coordinate Geometry"]),
       ("Geometry", 25, 69, ["Introduction to Euclid's Geometry: Axioms and Postulates",
                             "Lines and Angles", "Triangles - Congruence Theorems",
                             "4-gons (Quadrilaterals)", "Circles"]),
...
for i, (name, marks, periods, chs) in enumerate(U9M, 1):
    ...
    for ch in chs:
        p, obj = OBJ9M[ch]
        tops.append(topic(ch, objectives=obj,
                          source_ref=ref("cbse-9-maths", page=p, section="Unit %s, chapter %s" % (i, ch))))
    units.append(unit(name, ref("cbse-9-maths", page=4, section="Course structure Class IX, unit %d" % i),
                      topics=tops, marks=marks, periods=periods))
write("cbse/class-9-mathematics.json", syllabus(**FW, level="Class 9", level_order=9,
    subject="Mathematics", status="verified", documents=[D9M], units=units,
...
U9S = [("World of Living", 27, [(2, "Cell"), (3, "Tissues"), (11, "Reproduction"), (12, "Diversity")]),
       ("Matter - Its Nature and Behaviour", 25,
        [(5, "Exploring Mixtures and their Separation"), (8, "Structure of an Atom"),
         (9, "Atoms and Molecules")]),
       ("Motion, Force, Work and Sound", 23,
        [(4, "Motion"), (6, "Force and Laws of Motion"), (7, "Work, Energy and Simple Machines"),
         (10, "Sound")]),
...
for i, (name, marks, chs) in enumerate(U9S, 1):
    ...
    for num, ch in chs:
        b = B9S[ch]
        tops.append(topic(ch, objectives=[tidy(o) for o in b["objectives"]], chapter_number=num,
                          source_ref=ref("cbse-9-science", page=b["page"], section=ch)))
    units.append(unit(name, ref("cbse-9-science", page=4, section="Course structure Class IX, unit %d" % i),
                      topics=tops, marks=marks))
write("cbse/class-9-science.json", syllabus(**FW, level="Class 9", level_order=9,
    subject="Science", status="verified", documents=[D9S], units=units,
