...
STATES = [
 dict(framework_id="telangana", framework_name="Board of Secondary Education, Telangana",
      region="Telangana", aliases=["Telangana State Board", "TS SSC", "BSE Telangana"],
      docs=[("ts_scert_home", "State Council of Educational Research and Training, Telangana - "
             "site index", "State Council of Educational Research and Training, Telangana"),
            ("ts_bse_home", "Board of Secondary Education, Telangana - site index",
 ...
 dict(framework_id="andhra-pradesh", framework_name="Board of Secondary Education, Andhra Pradesh",
      region="Andhra Pradesh", aliases=["Andhra Pradesh State Board", "AP SSC", "BSE AP"],
      docs=[("ap_bse_home", "Board of Secondary Education, Andhra Pradesh - site index",
 ...
 dict(framework_id="maharashtra", framework_name="Maharashtra State Board of Secondary and Higher "
                                                 "Secondary Education",
      region="Maharashtra", aliases=["Maharashtra State Board", "MSBSHSE", "SSC Maharashtra",
                                     "Balbharati"],
      docs=[("mh_balbharati_home", "Maharashtra State Bureau of Textbook Production and "
 ...
 dict(framework_id="karnataka", framework_name="Karnataka School Examination and Assessment Board",
      region="Karnataka", aliases=["Karnataka State Board", "KSEAB", "SSLC Karnataka", "KTBS"],
      docs=[("ka_textbooks", "Karnataka Textbook Society textbook portal - index",
...
NEXT = ("Next step: run the discovery job (docs/CURRICULUM.md section 4) with a browser-capable "
...
for st in STATES:
    ...
    for cls in range(6, 11):
        for subject in SUBJECTS:
            write("%s/class-%d-%s.json" % (st["framework_id"], cls, subject.lower().replace(" ", "-")),
                  syllabus(framework_id=st["framework_id"], framework_name=st["framework_name"],
                           framework_kind="state", country="IN", region=st["region"],
                           aliases=st["aliases"], version=VERSION,
                           level="Class %d" % cls, level_order=cls, subject=subject,
                           status="provisional", documents=docs, units=[],
                           note=st["blocker"] + " " + NEXT))

# ---------------------------------------------------------------- ICSE 6-8
ICSE_NOTE = ("CISCE publishes a syllabus only for the ICSE examination (classes IX and X) and the "
...
for cls in range(6, 9):
    for subject in ["Mathematics", "Science", "Social Studies"]:
        write("icse/class-%d-%s.json" % (cls, subject.lower().replace(" ", "-")),
              syllabus(framework_id="icse",
                       framework_name="Indian Certificate of Secondary Education (CISCE)",
                       framework_kind="national", country="IN",
                       aliases=["ICSE", "CISCE",
                                "Council for the Indian School Certificate Examinations"],
                       version=VERSION, level="Class %d" % cls, level_order=cls, subject=subject,
                       status="provisional",
                       documents=[doc("icse_maths",
                                      "Mathematics (51), ICSE examination year 2027",
                                      "Council for the Indian School Certificate Examinations")],
                       units=[], note=ICSE_NOTE))
