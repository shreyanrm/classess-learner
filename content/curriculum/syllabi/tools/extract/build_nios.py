...
REPAIRS = {
    ...
    "People\u2019s Participa in the Democratic Process":
...
FW = dict(framework_id="nios", framework_name="National Institute of Open Schooling",
          framework_kind="open", country="IN", aliases=["NIOS", "Open school"])
PUB = "National Institute of Open Schooling"
VERSION = "2026-27"
NOTE = ("NIOS states its syllabus as a bifurcation table: one row per module, with that module's "
...
for key, course, level, order, stage in SPEC:
    ...
    d = doc(key, "Bifurcation of syllabus, %s, NIOS %s course" % (course, stage), PUB)
    did = key.replace("_", "-")
    mods = parse(key)
    units = []
    for m in mods:
        tops = [topic(REPAIRS.get(l["title"], l["title"]), lesson_number=l["number"],
                      source_ref=ref(did, page=m["page"],
                                     section="Module %s: %s" % (m["number"], m["title"])))
                for l in m["lessons"]]
        units.append(unit(REPAIRS.get(m["title"], m["title"]), ref(did, page=m["page"], section="Module %s" % m["number"]),
                          topics=tops, unit_number=m["number"]))
    write("nios/%s-%s.json" % (level.lower().replace(" ", "-"),
                               subject.lower().replace(" ", "-")),
          syllabus(**FW, version=VERSION, level=level, level_order=order, subject=subject,
                   status="verified", documents=[d], units=units, note=NOTE,
                   extra={"course_code": code, "stage": stage}))

# English: fetched, but the bifurcation table is not module-numbered.
ENG_NOTE = ("The NIOS English bifurcation table was fetched (see the document below) but it is "
...
for key, course, level, order, stage in [
    ...
    d = doc(key, "Bifurcation of syllabus, %s, NIOS %s course" % (course, stage), PUB)
    write("nios/%s-english.json" % level.lower().replace(" ", "-"),
          syllabus(**FW, version=VERSION, level=level, level_order=order, subject="English",
                   status="provisional", documents=[d], units=[], note=ENG_NOTE,
