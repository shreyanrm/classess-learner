...
>
> If a child seems to be in distress, Wobo stops the lesson, says plainly that I am not a counsellor, and points to people who can help.
>
> We do not have humans reading conversations as they happen. Safety checks are automatic. Flagged material can be reviewed by a small, trained team.
>
...
4. **Points to real help.** Wobo names the emergency number for the learner's country, offers the option of telling a trusted adult and helps the learner think about who that might be, and, where we hold a helpline for that country, shows it. Wobo does not diagnose and does not counsel. [REVIEW: the per-country helpline list this step depends on does not exist yet. It is the single most consequential unbuilt thing in this folder, because this is the document that gets read after an incident. Before publication it needs a named owner, a source for each country we sell in, at least one service per country that is free, confidential and available to children, a review cadence, a check that each number is still live, and a written fallback for any country not yet covered, which is the local emergency number and telling a trusted adult. Until the list exists, this step must not be described anywhere as a maintained list.]
...
- Wobo can still be wrong. The flag control is on every piece of content, and `community-and-flags.md` explains what happens to a flag.

## 5. How safety is enforced

- Checks run on the way in and on the way out of every turn, in our own systems rather than at the learner's device, so they cannot be switched off.
- The limits are stricter for a younger learner, and age is derived on the server, not asserted by the app.
