...
**WHERE:** the Learn surface — OpenerCard + CanvasSurface + (on demand) WoboPanel + RevealPanel.
...
4. **Stuck? → Wobo** — if struggle crosses a threshold (misses, time, frustration signal), Wobo is offered/steps in: perceives the
   working, gives a **graduated hint** (ask-before-tell), never the answer. (See `04-wobo-flow.md`.)
5. **Reveal (fading)** — once they've grappled, the reveal arrives: Wobo's annotation on the canvas (primary), or a short nugget/reading
...
switches pull alternate verified representations from the content cache or generate+verify on miss; Wobo via the gateway.

**EVENTS:** `learn.node.entered.v1`, `learn.opener.posed.v1`, `learn.attempt.submitted.v1`, `wobo.hint.escalated.v1`,
...
**AI CALLS:** `generate.opener`, `verify.*`, `orchestrator.step` (switch/forward/frustration), `wobo.turn`, `grade.attempt`.
**STATES:** monochrome-enter · pose · struggle · stuck→wobo · reveal · modality-switch · practice-gate · ignite · constellation · advance ·
(reduced-motion equivalents for ignite/constellation).
**GUARDRAILS:** never explain-first; never serve unverified content; Wobo never hands the answer; mastery (not vibes) ends the loop.
