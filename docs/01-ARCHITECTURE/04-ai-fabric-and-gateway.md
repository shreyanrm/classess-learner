...
- **Two tracks, never conflated:**
  ...
  - **Track 2** — proprietary fine-tuned models + **edge SLMs** (the margin and the moat). Most Wobo turns and routine
...
## 4) Wobo — the five capabilities (the signature; see `02-DESIGN/02-wobo.md` for form/behaviour)
Wobo is built as five composable capabilities behind the gateway:
...
3. **Reason grounded in correctness** — never free-styles math; proposes steps, the verifier checks before Wobo commits to them.
