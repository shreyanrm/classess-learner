...
- **Phase 0 — Foundation.** Repo, monorepo packages, the event contract, the KGtoPG contract seed, Supabase (RLS-ready, auth-deferred-but-wired),
  the model gateway + verifier skeleton, the design/motion/Vidya packages scaffolded, CI. No product features yet — the spine.
- **Phase 1 — The Atom (the proof gate).** Linear equations in one variable, taught end to end: the learn loop, the canvas, Vidya transcendent
...
- **Phase 3 — Breadth.** Widen content (full middle-school math, then beyond), more surfaces (exam-as-subject, belonging/social if/when),
  Vidya widened beyond the atom, Remotion nugget library, full design polish across all pages.
...
- Design system, motion system, Vidya spec + **Vidya-cute license** respected on every surface.
