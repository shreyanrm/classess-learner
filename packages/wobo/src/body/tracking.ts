/**
 * Where she looks. Nothing here is placed by pixels in her own space: a caller hands over a real
 * rectangle on screen — a focus region, a registry target's box, or the pointer as a zero-size
 * rect — and this turns it into a normalised gaze vector and the rig-unit offsets that move her
 * eyes and lean her head.
 *
 * The rule (BOARD.md §3, wave 5): she tracks what is IN FOCUS; the pointer is only the fallback
 * when nothing is.
 */
...
/**
 * How far a target has to be before she is looking as hard as she can. It scales with her own size
 * so a small docked orb does not stare wide-eyed at something just beside it, and never collapses
 * to zero when she has not been laid out yet.
 */
...
/**
 * The normalised gaze vector from her own box to a point, each axis clamped to -1..1.
 * (0, 0) means she is looking straight ahead.
 */
...
/** The same, for a focus region: she looks at the middle of what is in focus. */
...
/** The gaze vector as a head lean in rig units — she turns toward what she is looking at. */
export function leanOffset(v: Point): number {
  return clamp1(v.x) * LEAN_REACH;
}

/**
 * Pick what she should look at, in priority order: the focus region, then an explicit gaze the
