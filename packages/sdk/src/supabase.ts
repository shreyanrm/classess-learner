 ...
 * ponytail: no @supabase/supabase-js — add it when auth flows/realtime land (Phase 4).
 */
...
export interface SupabaseRestConfig {
  ...
  anonKey: string;
  /** JWT whose sub = subject_id; RLS keys every row to it. Absent => anon role (reads only fail closed). */
  accessToken?: string;
}
