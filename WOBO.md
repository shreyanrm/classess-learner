...
The ink-visor wobot (DESIGN.md §the body). A round body in ink — `#14142B` on cream, `#F3F0E8` on night — carrying a visor in the tone the body is not, two Wobo-blue eyes (`#2B45FF`, lifting to `#7C8CFF` on night) and a pen tip in the same blue, with a half-pixel opposite-tone hairline that keeps the silhouette crisp on any ground. The first design’s soft glowing orb body is retired; that warmth is an earned accent now, never the character. Real physical personality survives the change — weight and squash, anticipation before motion, overshoot and settle, constant idle life (blinks, glances, boredom, stretches) so Wobo is never frozen, and a gaze that follows the pointer or the pen. Rebuilt from zero to a higher bar than any mascot in the category — the Koji quality bar, exceeded. Joyful, alive, never saccharine, never noisy. A body, not a gender (WOBO-PLAN §19): nothing in the rig, the palette, or the voice signals a boy or a girl, Wobo is a wobot and neither, and every word written about the character leads with the name — they/them only where a pronoun is unavoidable.

**States, each with distinct body language:** idle (a settled breath and the small idle life), listening (leaning in toward the learner), thinking (a gentle inward pulse), drawing (the pen tip out, the body angled at what Wobo is annotating), celebrating (a bright squash-and-pop on genuine mastery, tied to the ignite event), resting (a slow calm breath that sanctions planned rest, never guilt).

**Tech:** the rig is SVG driven by spring channels in one animation loop — twenty expressions and a scenes registry the brain can cue — with Lottie for set-piece flourishes and Framer Motion for interface transitions. Wobo's reactions are driven by state, never canned loops. The command palette (Cmd-K) reaches any action; the docked body and hold-to-talk reach Wobo's voice.
