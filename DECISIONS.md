...
- **Explainable intelligence as product law.** Every Wobo recommendation/proactive chip carries: why it appeared, the evidence behind it, a confidence band, and what ignoring it costs. Nothing she suggests is a black box. (Folded into the wave-4 orchestrator missions.)
...
- **2026-09-02 · Rebrand: Classess Learner → Wobo.** The product is now **Wobo**, and the tutor formerly called Vidya is now **Wobo** too — one name for the app and for the tutor who lives in it. She keeps her pronouns, body, voice, and behaviour; only the name changed. Identifiers moved with it: the package is `@classess/wobo`, the capability is `wobo.turn` (the legacy `vidya.turn` name is aliased on the gateway for one release so the deployed bundle keeps working), and the law docs are `WOBO.md` and `WOBO-CAPABILITIES.md`. The **Classess** name is retained for the ecosystem and its B2B products (Classess Teacher, Classess School) and for internal identifiers. Contract event types carrying the old name were renamed pre-launch: no production event lake existed and the Supabase project was paused at the time, so no replay or migration was owed.
