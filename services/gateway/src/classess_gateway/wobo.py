...
This is Wobo's orchestrator seam (WOBO.md §6): every request she receives is classified into
...
def mock_wobo_turn(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # The very first meeting: her introduction, verbatim, before anything else can claim the turn.
...
def is_first_meeting(payload: dict[str, Any]) -> bool:
    """Is this the very first time this learner meets her? The web client marks the onboarding
    turn — either ``payload["first_meeting"]`` or ``context.turn.firstMeeting`` — and only then
    does she introduce herself with the owner's exact line. Absent or false means a returning
